/*
正点原子战舰STM32开发板适用于RTGUI底层TFTLCD驱动
...
void LCD_FSMCConfig(void) //液晶屏FMSC驱动初始化配置
{
...
void rt_hw_lcd_get_pixel(char* pixel, int x, int y)
{
    unsigned short p;
	u16 r=0,g=0,b=0;
	if(x>=lcddev.width||y>=lcddev.height)p = 0;	//超过了范围,直接返回		   
	LCD_SetCursor(x,y);	
	...
	//if(lcddev.id==0X9325||lcddev.id==0X4535||lcddev.id==0X4531||lcddev.id==0X8989||lcddev.id==0XB505)p = r;//这几种IC直接返回颜色值
	//else if(lcddev.id==0X9341)
	p = (((r>>11)<<11)|((g>>10)<<5)|(b>>11));	//ILI9341需要公式转换一下
...
void rt_hw_lcd_draw_hline(const char* pixel, int x1, int x2, int y)
{
    ...
    //LCD_WriteReg(0x0003,(1<<12)|(1<<5)|(1<<4)|(0<<3));  //原版
//	LCD_Scan_Dir(L2R_U2D);	//使用的是9341这个奇葩，所以扫描方向得另外弄
...
{
//     /* [5:4]-ID~ID0 [3]-AM-1垂直-0水平 */
//     //LCD_WriteReg(0x0003,(1<<12)|(1<<5)|(0<<4)|(1<<3));  //原版
// 	LCD_Scan_Dir(U2D_R2L);	//使用的是9341这个奇葩，所以扫描方向得另外弄
