...
void _draw_default(struct rtgui_object *object, rtgui_event_t *event)
{
    ...
    /* 显示提示 */
    rtgui_dc_draw_text(dc, "点击按钮开始绘图测试", &rect);
...
rtgui_container_t *demo_view_benchmark(void)
{
	...
	//这里参考百为开发板的例程，在视图上添加按钮来开始或者停止测试
	//获取可绘图区域
	...
	rect.y1 = rect.y2 - 25;
	/* 创建按钮 */
	start_stop_btn = rtgui_button_create("开始/停止");
	/* 设置onbutton动作到start_stop函数 */
	...
	//设置绘图测试的处理事件到benchmark_event_handler函数
